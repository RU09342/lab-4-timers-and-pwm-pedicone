...
void main(void)
{
    ...
    P2DIR &= ~btn;      // configure button as input
    P2REN |= btn;       // add pull up resistor
    P2OUT |= btn;
   ...
   {
       P2IE     &= ~btn;          // Disable the button interrupt
   ...
   __interrupt void Timer_A (void)
   {
       ...
       TA0R = 0;            //Clear timer
       P2IE |= btn;        //Re-enable button interrupt
       P2IFG &= ~btn;      //Clear the button interrupt flag
   }
