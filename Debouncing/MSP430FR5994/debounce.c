...
void main(void)
{
    ...
    P5DIR &= ~btn;      // configure button as input
    P5REN |= btn;       // add pull up resistor
    P5OUT |= btn;
   ...
   {
       P5IE     &= ~btn;          // Disable the button interrupt
   ...
   __interrupt void Timer_A (void)
   {
       ...
       TA0R = 0;            //Clear timer
       P5IE |= btn;        //Re-enable button interrupt
       P5IFG &= ~btn;      //Clear the button interrupt flag
   }
